...
Windowsでのビルド時の挙動としては、MECAB_LIBS（指定しない場合`/mecab`になる）にあるlibmecab.dllを参照してビルドしますが、MECAB_LIBSにlibmecab.dllがない場合、あらかじめ用意されているlibmecab.dllをMECAB_LIBSにコピーします。`--no-multiarch`を指定しない場合、ビルドする過程で32bit向けのlibemecab.dllが意図せず64bit向けのlibmecab.dllで上書きされるおそれがあるため、Windowsではこのインストールオプションを忘れずに指定してください。上で紹介したやり方で`C:/mecab`にlibmecab.dllを配置している場合、MECAB_LIBSはとくに指定しなくてもインストールできます。
