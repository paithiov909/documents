...
サーバーコマンドを非同期で叩くことができます。kagomeのサーバーモードの利用の仕方については公式の案内にしたがってください。デフォルトでは環境変数`KAGOME_URL`に設定したURLにアクセスします。`Sys.setenv(KAGOME_URL = "http://localhost:6060")`などとして環境変数を設定するか、アクセスしたいURLを`RcppKagome::queue(url = "http://localhost:6060")`のように明示的に指定してください。
