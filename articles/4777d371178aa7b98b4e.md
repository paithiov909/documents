...
## リストの各要素は、表層系（surface form）と素性情報の1番目（IPA辞書では「品詞」）を'/'で区切ってつなげた文字列ベクトルになる。
...
## pos列・subtype列は素性情報の1~2番目（IPA辞書では「品詞」と「品詞細分類1」）、analytics列は素性情報の8番目（IPA辞書の「読み」）だが、
...
- OS・プラットフォームを問わず、渡したcharacter vectorを`stringi::stri_to_encutf8`でUTF-8に変換するように。
...
[他の記事](https://mana.bi/wiki.cgi?page=RcppMeCab%A4%C8RMeCab%A4%CE%C8%E6%B3%D3)ですでに紹介されているように、従来のRcppMeCabについてはRMeCabの同等の処理のほうが速いケースがありましたが、MeCabそのものの解析速度には差はないため、RパッケージからMeCabを利用する際にはC/C++部分の書き方によってパフォーマンスに差が出ます。筆者のforkではそのあたりはある程度リファクタリングされているため、単純に文字列ベクトルを形態素解析するかぎりではRcppMeCabのほうが速いです。
...
なお（きちんと確認していませんが）、[mecab-ko-msvc](https://github.com/Pusnow/mecab-ko-msvc)についても元のMeCabのソースコードをMicrosoft Visual C++でビルドできるように弄った野良ビルドのひとつで、生成されているバイナリとしては他の日本語向けの野良ビルドとほとんど同じものだと思われます。日本語向けと韓国語向けとの主な違いは、デフォルトで見に行く辞書の場所くらいなので、たとえば日本語と韓国語を切り替えたいからという場合であっても、後からRcppMeCabパッケージのインストールし直したりする必要はありません。
...
```r
# Sys.setenv(MECAB_LIBS = "/mecab/bin") # Windowsで特定の場所にあるlibmecab.dllを参照させたい場合
...
Windowsでのビルド時の挙動としては、MECAB_LIBS（指定しない場合`/mecab`になる）にあるlibmecab.dllを参照してビルドしますが、MECAB_LIBSにlibmecab.dllがない場合、あらかじめ用意されているlibmecab.dllをMECAB_LIBSにコピーします。上で紹介したやり方で`C:/mecab`にlibmecab.dllを配置している場合、MECAB_LIBSなどを指定しなくてもインストールできます。
